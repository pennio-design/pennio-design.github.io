...
- **The headshot is built, not hand-cropped.** `paul-oyatowo.webp` is a circle inscribed in a 400px square: cream `#F5F1EB` fill, a 5px `#F26522` ring flush to the edge, transparency outside so the disc works on either theme. Rebuild it from a background-removed portrait with `python3 scripts/build-headshot.py <cutout.png>` rather than editing the webp; the framing constants and the reason the ring is an annulus are in `scripts/README.md`. It is referenced twice, at `index.html` line 767 and as `Person.image` in the `@graph`, both at 400x400, so a rebuild is a drop-in and needs no markup change.
